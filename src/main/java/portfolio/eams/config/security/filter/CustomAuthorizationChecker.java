    ...
    public AuthorizationDecision check(Supplier<Authentication> authentication, RequestAuthorizationContext object) {
        ...
        if (!auth.getPrincipal().equals("anonymousUser")) {
            ...
            if (reqUri.equals("/dashboard")) { 
                ...
                String roleNm = auth.getAuthorities().iterator().next().toString();
                Role role = roleRepo.findByRoleNm(roleNm).orElse(null);

                if (role != null) {
                    // roleId 를 통해 메뉴 읽기권한 유무 검사
                    for (MenuRoleAcs acs : menuRoleAcsRepo.findByRole_Id(role.getId())) {
